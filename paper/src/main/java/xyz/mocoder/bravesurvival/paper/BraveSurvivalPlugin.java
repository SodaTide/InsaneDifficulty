...
public class BraveSurvivalPlugin extends JavaPlugin implements Listener {
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if (event.getEntity() instanceof Player player) {
            ...
            // 岩浆伤害 (使用event.setDamage而不是player.damage)
    ...
    public void onPlayerMoveForVillage(PlayerMoveEvent event) {
        ...
        if (ConfigManager.getCombatConfig().has("bad_omen_on_village_enter") && 
            ...
            double chance = ConfigManager.getCombatConfig().has("bad_omen_chance") ? 
                ConfigManager.getCombatConfig().get("bad_omen_chance").getAsDouble() : 0.33;
            if (Math.random() < chance * 0.001) { // 降低触发频率
                Player player = event.getPlayer();
                // 检查附近是否有村庄结构
                if (isNearVillage(player.getLocation())) {
    ...
    private boolean isNearVillage(org.bukkit.Location location) {
        // 简化实现：检查附近是否有村民
        for (Entity entity : location.getWorld().getNearbyEntities(location, 100, 50, 100)) {
